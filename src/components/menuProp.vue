            ...
            <div class="ml-auto relative max-w-xs w-full h-full bg-white shadow-xl py-4 pb-12 flex flex-col overflow-y-auto">
                ...
                <div class="mt-4 border-t border-gray-200">
                    <Disclosure as="div" v-for="section in menuProp" :key="section.id" class="border-t border-gray-200 px-4 py-6" v-slot="{ open }">
                        <div v-show="section.value.visible !== false">
                            ...
                            <div class="space-y-4">
                                <div v-for="(option, optionIdx) in section.value.items" :key="option.id">
                                    ...
                                    <div v-if="option.type == 'check'" class="flex items-center">
                                        <input v-if="'change' in option" :id="`check-${option.id}-${optionIdx}`" :name="`${option.id}[]`" type="checkbox" v-model="option.checked" :checked="option.checked"
                                            class="h-4 w-4 border-gray-300 rounded text-indigo-600 focus:ring-indigo-500" @click="changeState(option.change, $event)" />
                                        <input v-else :id="`check-${option.id}-${optionIdx}`" :name="`${option.id}[]`" type="checkbox" v-model="option.checked" :checked="option.checked"
                                        ...
                                        <label :for="`check-${option.id}-${optionIdx}`" class="ml-3 min-w-0 flex-1 text-gray-500">
                                            {{ option.label }}
                                        </label>
                                    </div>
                                    <!-- Select -->
                                    ...
                                    <div v-else-if="option.type == 'text'" >
                                        <div v-if="'eshijo' in option && option.eshijo">
                                            <label v-bind:for="`subtext-${optionIdx}-${option.id}`" class="block text-sm font-medium text-gray-700 transform translate-x-6"> {{ option.label }} </label>
                                            <input type="text" v-model="option.value" :name="`subtext-${optionIdx}-${option.id}`" class="text-xs rounded shadow transform translate-x-6" 
                                                :maxlength="option.max" :readonly="option.readonly" :disabled="'enabled' in option && !option.enabled" />
                                        </div>
                                        <div class="block" v-else>
                                            <label v-bind:for="`text-${optionIdx}-${option.id}`" class="block text-sm font-medium text-gray-700"> {{ option.label }} </label>
                                            <input type="text" v-model="option.value" :name="`text-${optionIdx}-${option.id}`" class="text-xs rounded shadow w-full" 
                                                :maxlength="option.max" :readonly="option.readonly" :disabled="'enabled' in option && !option.enabled" />
                                        </div>
                                    </div>
                                    <!-- Button -->
